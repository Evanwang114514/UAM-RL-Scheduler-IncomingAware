...
乘客在起点被激活后，系统首先计算起点到终点的地面直线距离。对于短途出行乘客（即地面出行时间小于任何可行飞行方案），系统直接分配地面交通，乘客以 CAV 速度前往终点，不进入起降场排队与飞行流程。

对于中长途出行乘客，系统首先采取最短路分配终点起降场，随后系统接入带有排队感知的模型为其分配起始起降场。乘客以地面 CAV 速度从起点移动至起飞机场，进入排队队列等待空闲 eVTOL。
