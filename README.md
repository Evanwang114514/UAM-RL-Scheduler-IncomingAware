...
（1）PPO 方法优于规则基线：在三种 eVTOL 配置下，本文 PPO 方法均优于最短路径与最短路惩罚基线。8/12/16 架配置下相比最短路径分别降低 41.0%/40.8%/22.7%，相比最短路惩罚分别降低 14.8%/14.6%/13.9%。强化学习通过与环境交互学习动态避堵策略，优于线性固定规则的惩罚方法。

（2）排队预判特征有效：通过消融实验对比基线 PPO（12 维，不含飞往 eVTOL 数量）与本文方法（20 维，含飞往 eVTOL 数量），AWT 在 8/12/16 架配置下分别降低 7.3%/7.8%/10.8%。观察飞往起降场的 eVTOL 数量使模型更能够预测每个起降场的排队趋势。

（3）运力紧张场景下模型优势更显著：8 架 eVTOL（运力紧张）时本文方法相比基线 PPO 的 ATT 降低 6.3%，而 16 架（运力充足）时仅降低 4.8%。在运力紧张时排队更容易累积，模型通过排队预判与动态避堵获得的收益更大。

（4）地面-飞行分类机制有效：本文方法在三种配置下的地面完成人数分别为 351/345/332 人，显著高于最短路径与最短路惩罚。短途乘客被合理分配地面交通，有效缓解了起降场排队压力。
