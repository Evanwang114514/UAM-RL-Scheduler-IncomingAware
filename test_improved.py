...
from seven_choice_env_improved import SevenChoiceEnvImproved
...
def test_rl_model(model_path, passenger_file='data/passengers_800_test.csv', n_passengers=800, max_ticks=12000):
    print("=" * 120)
    print(f"新PPO测试 (20维) - {n_passengers}人 (测试集)")
    print("=" * 120)
    ...
    env.env.pending_passengers = []
    for p in all_passengers:
        env.env.pending_passengers.append({
            'origin': p['origin'],
            'dest': p['dest'],
    ...
    env.env.step_count = 0
    env.env.passenger_id = 1
    env.env.total_spawned = 0
    env.env.passenger_origin = None
    env.env.passenger_dest = None
    env.env.walking_remaining = [-1] * n_passengers
    env.env.passenger_spawn_step = {}
    env.env.passenger_arrive_step = {}
    env.env.passenger_takeoff_step = {}
    env.env.passenger_final_step = {}
    env.env.queues = [[] for _ in range(8)]
    env.env.airport_aircraft = [[] for _ in range(8)]
    env.env.flying_aircraft = [[] for _ in range(8)]
    env.env.walking_passengers = []
    env.env.completed_passengers = []

    for i in range(env.env.num_aircraft):
        env.env.airport_aircraft[i % 8].append(i)

    env.env._load_next_passenger()
    ...
    while step < max_ticks:
        all_takeoff = (
            len(env.env.completed_passengers) >= n_passengers and
            len(env.env.walking_passengers) == 0 and
            sum(len(q) for q in env.env.queues) == 0
        )
        if all_takeoff:
            break

        if env.env.passenger_origin is not None and env.env.passenger_dest is not None:
            origin = env.env.passenger_origin
            dest = env.env.passenger_dest
            pid = env.env.current_passenger_id
        ...
        if step % 500 == 0:
            for i, p in enumerate(all_passengers):
                pid_check = p['id']
                if p['info']['is_ground']:
                    if pid_check in env.env.passenger_final_step:
                        completed_status[i] = 1
                else:
                    if pid_check in env.env.passenger_takeoff_step:
                        completed_status[i] = 1

            completed_count = len(env.env.completed_passengers)
    ...
    if step >= max_ticks:
        ...
        print(f"   已完成 {len(env.env.completed_passengers)}/{n_passengers} 人")
        print(f"   走路中 {len(env.env.walking_passengers)} 人")
        print(f"   队列中 {sum(len(q) for q in env.env.queues)} 人")
        print(f"   飞行中 {sum(len(f) for f in env.env.flying_aircraft)} 架")
    ...
    if completed_count > 0:
        ...
        for cp in env.env.completed_passengers:
        ...
        print(f"  (AGT+AWT+AAT = {np.mean(all_ground) / 10 + np.mean(all_wait) / 10 + np.mean(all_flight) / 10:.2f} 分钟)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str,
                        default='models/improved_ppo/final_model.zip',
                        help='模型路径')
