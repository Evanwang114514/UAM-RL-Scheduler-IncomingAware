"""
seven_choice_env_improved.py - 改进版七选一决策环境
动作 0-6: 选择前7个最近的起降场
状态: 20维 (8队列 + 8飞往飞机 + 4位置)
奖励: R = -ATT
"""
...
class SevenChoiceEnvImproved(gym.Env):
    """
    改进版七选一决策环境包装器
    原始动作空间: 57 -> 压缩后: 7
    状态空间: 20维 (8队列 + 8飞往飞机 + 4位置)
    奖励: R = -ATT
    """
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            -np.inf, np.inf, (20,), dtype=np.float32
        )
    ...
    def _get_state(self):
        """获取状态 (20维): 8队列 + 8飞往飞机 + 4位置"""
        queues = [len(q) for q in self.env.queues]
        max_q = max(queues) + 1
        queue_norm = [q / max_q for q in queues]

        flying = [len(f) for f in self.env.flying_aircraft]
        max_f = max(flying) + 1
        flying_norm = [f / max_f for f in flying]
        ...
        state = np.array(queue_norm + flying_norm + pos, dtype=np.float32)
        return state
