...
def run_shortest_baseline(n_passengers=800, passenger_file='data/passengers_800_test.csv'):
    print("=" * 120)
    print(f"最短路径基线测试 - {n_passengers}人 (测试集)")
    ...
    while step < max_ticks:
        ...
        if env.passenger_origin is not None:
            action_a, _ = get_shortest_action(env.passenger_origin, env.passenger_dest)
        else:
            action_a = 56
...
if __name__ == "__main__":
    ...
    run_shortest_baseline(args.n_passengers, args.passenger_file)